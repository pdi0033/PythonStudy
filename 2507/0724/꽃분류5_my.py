...
def make_subset():   # make_subset("train", 0, 1000)
    subset_name_List = ["train", "validation", "test"]
    for category in flower_names:
        ori_dir = original_dir/category

        train_dir = new_base_dir/"train"/category     # WindowsPath 라는 특별한 객체, str 아님.
        os.makedirs(train_dir, exist_ok=True)     # 디렉토리가 없을 경우 새로 디렉토리를 만들어라
        validation_dir = new_base_dir/"validation"/category
        os.makedirs(validation_dir, exist_ok=True)    
        test_dir = new_base_dir/"test"/category
        os.makedirs(test_dir, exist_ok=True)    

        i = 0
        dataList = os.listdir(ori_dir)
        flower_count = len(dataList)
        for fname in dataList:
            if i < flower_count//2:
                shutil.copyfile(src=ori_dir/fname, dst=train_dir/fname)
            elif i < int(flower_count * 3/4):
                shutil.copyfile(src=ori_dir/fname, dst=validation_dir/fname)
            else:
                shutil.copyfile(src=ori_dir/fname, dst=test_dir/fname)
            i += 1
...
def deeplearning():
    ...
    with open("꽃분류_사전학습.bin", "wb") as file:
