# pip install Korpora
# 네이버 영화평을 준다.
# 미리 학습된 데이터셋을 사용한다. Word2Vec을 사용한다.
...
def study():
    ...
    embedding_dim = 300     # 대충, 특별히 사전에 학습된 내용을 사용하는 것이 아니면 차원을 마음대로 줄 수 있다.

    # limit 안 주면 20만 개라서 메모리 fault
    korean_word2vec_path = "../../data/cc.ko.300.vec"
    korean_word_vector = KeyedVectors.load_word2vec_format(korean_word2vec_path, binary=False,
                                                           encoding='utf-8-sig', limit=50000)  # text 파일임
    print(korean_word_vector.vector_size)
    embedding_dim = korean_word_vector.vector_size

    # 임베딩 행렬을 구축한다.
    embedding_matrix = np.zeros( (voca_size, embedding_dim) )   # np.zero함수가 매개변수를 tuple로 받아간다. 그래서 괄호가 꼭 필요하다.
    # 2차원 임베딩 배열 만들기
    # 자연어 0.2, 0.1, 0.2, ......
    # 감동   0.1
    for i, word in enumerate(vocabulary):
        if i < voca_size:   # max_tokens를 벗어나지 않는 범위 내에서
            try:
                embedding_vector = korean_word_vector[word]     # 파일에 있는 벡터값 읽어와서
    ...
    x = layers.Embedding(
        ...
        embeddings_initializer=keras.initializers.Constant(embedding_matrix),       # 초기값을 랜덤=>학습된 데이터로 바꾸고
        trainable=False,    # 현재 들고 온 가중치를 써라
    )(inputs)
    ...
    callbacks = [
        keras.callbacks.ModelCheckpoint("korean_rnn_model2.keras", 
                                        save_best_only=True,
    ...
    history = model.fit(train_ds_vectorized,
                        ...
                        epochs=5,
                        callbacks=callbacks)
    with open("korean_rnn_history2.bin", "wb") as f:
        pickle.dump(history.history, f)
    print("완료")

def predict():
    model = keras.models.load_model("korean_rnn_model2.keras")
    with open("korean_rnn_history2.bin", "rb") as f:
