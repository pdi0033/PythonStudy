...
input_box = driver.find_element(By.NAME, "query")   # 내가 name 속성을 이용해서 찾을 거다. name이 q다.
...
btnNews = driver.find_element(By.CSS_SELECTOR, "#lnb > div.lnb_group > div > div.lnb_nav_area._nav_area_root > div > div.api_flicking_wrap._conveyer_root > div:nth-child(8) > a")
btnNews.click()
...
while True:     # 무한루프
    ...
    ul = doc.find("ul", {"class":"list_news _infinite_list"})
    liList = ul.find_all("div", {"class":"sds-comps-vertical-layout sds-comps-full-layout JYgn_vFQHubpClbvwVL_"})
    for li in liList:
        print(li.text)
        if i > 10:
            break
